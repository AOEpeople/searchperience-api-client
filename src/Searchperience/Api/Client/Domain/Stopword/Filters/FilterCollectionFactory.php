...
namespace Searchperience\Api\Client\Domain\Stopword\Filters;
	...
	protected function validateFilterArguments($filters) {
		...
		foreach($filterNames as $filterName)  {
			if(!in_array($filterName,$this->allowedFilters)) {
				throw new \Searchperience\Common\Exception\UnexpectedValueException('Could not handle filter '.$filterName. ' for Stopword entity. Is this filter allowed?');
			}
		}
		return true;
	}
